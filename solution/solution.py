...
def call_openai(
    ...
    from openai import OpenAI  # Import bên trong hàm như gợi ý
    client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    count_input = count_tokens(prompt, model=model)
    count_output = count_tokens(response, model=model)
    pricing = PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gpt-4o"])
    input_cost = count_input / 1000 * pricing["input"]
    output_cost = count_output / 1000 * pricing["output"]
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        messages : list[ChatCompletionMessageParam] = [
            {"role": "system", "content": "Bạn là trợ giảng thân thiện của khóa AI, trả lời ngắn gọn bằng tiếng Việt."}
        ] + history
...
def run_assistant(
    ...
    while True:
        ...
        history = history[-6:]  # Keep only the last 3 turns (6 messages)
        num_turns += 1
        total_tokens += count_tokens(user_msg) + count_tokens(full_reply)
        cost_info = estimate_cost(user_msg, full_reply)
